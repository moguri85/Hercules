	...
	for(i = 0; i < 10; i++) {
		...
		if (inventory[index].amount < sd->deal.item[i].amount) {
			...
			if (battle_config.ban_hack_trade < 0) {
				chrif->char_ask_name(-1, sd->status.name, CHAR_ASK_NAME_BLOCK, 0, 0, 0);
			...
			// if we ban people
				...
				chrif->char_ask_name(-1, sd->status.name, CHAR_ASK_NAME_BAN, 0, battle_config.ban_hack_trade, 0); // type: 2 - ban (year, month, day, hour, minute, second)
				sockt->eof(sd->fd); // forced to disconnect because of the hack
				// message about the ban
